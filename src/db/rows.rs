...
pub async fn fetch_page(
    ...
    let rows = conn.client()
        .query(&sql, &[])
        .await
        .map_err(|e| DbError::Query { sql: sql.clone(), source: Box::new(e) })?;
    ...
    let headers: Vec<String> = if let Some(first) = rows.first() {
        ...
        let r = conn.client()
            .query(&info_sql, &[])
            .await
            .map_err(|e| DbError::Query { sql: info_sql.clone(), source: Box::new(e) })?;
        r.first()
    ...
    let est: i64 = conn.client()
        ...
        .await
        .map_err(|e| DbError::Query { sql: est_sql.clone(), source: Box::new(e) })?
        .map(|r| r.get(0))
        .unwrap_or(0);
