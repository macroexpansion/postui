...
pub fn col_to_string(row: &Row, idx: usize) -> String {
    ...
    match *col_type {
        ...
        Type::TEXT | Type::VARCHAR | Type::BPCHAR | Type::NAME => row
            ...
            .map_or_else(err, |o| o.unwrap_or_else(null)),
        Type::UUID => row
            .try_get::<_, Option<Uuid>>(idx)
            .map_or_else(err, |o| o.map(|u| u.to_string()).unwrap_or_else(null)),
        Type::TIMESTAMPTZ => row
            .try_get::<_, Option<DateTime<Utc>>>(idx)
            .map_or_else(err, |o| o.map(|t| t.to_rfc3339()).unwrap_or_else(null)),
        Type::TIMESTAMP => row
            .try_get::<_, Option<NaiveDateTime>>(idx)
            .map_or_else(err, |o| o.map(|t| t.to_string()).unwrap_or_else(null)),
        Type::DATE => row
        ...
        Type::JSON | Type::JSONB => row
            ...
            .map_or_else(err, |o| o.map(|j| j.to_string()).unwrap_or_else(null)),
        Type::NUMERIC => row
            .try_get::<_, Option<String>>(idx)
            .map_or_else(err, |o| o.unwrap_or_else(null)),
        Type::BYTEA => row
            .try_get::<_, Option<Vec<u8>>>(idx)
            .map_or_else(err, |o| o.map(hex).unwrap_or_else(null)),
        Type::TEXT_ARRAY | Type::VARCHAR_ARRAY => row
            .try_get::<_, Option<Vec<String>>>(idx)
            .map_or_else(err, |o| {
                o.map(|v| format!("{{{}}}", v.join(",")))
                    .unwrap_or_else(null)
            }),
        Type::INT4_ARRAY => row
            .try_get::<_, Option<Vec<i32>>>(idx)
            .map_or_else(err, |o| {
                o.map(|v| format!("{{{}}}", join_strs(&v)))
                    .unwrap_or_else(null)
            }),
        Type::INT8_ARRAY => row
