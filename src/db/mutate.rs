...
pub fn build_update(
    ...
    if pk.columns.is_empty() {
        return Err(DbError::Type(
            "table has no primary key — refusing UPDATE".into(),
        ));
    }
    let set = edits
        .iter()
        .map(|e| format!("{} = {}", quote_ident(&e.name), render(&e.value)))
        .collect::<Vec<_>>()
        .join(", ");
...
pub fn build_delete(schema: &str, table: &str, pk: &PrimaryKey) -> Result<String, DbError> {
    if pk.columns.is_empty() {
        return Err(DbError::Type(
            "table has no primary key — refusing DELETE".into(),
        ));
    }
    Ok(format!(
        "DELETE FROM {}.{} WHERE {};",
...
pub fn build_insert(schema: &str, table: &str, values: &[ColumnEdit]) -> Result<String, DbError> {
    if values.is_empty() {
        return Err(DbError::Type("no columns to insert".into()));
    }
    let cols = values
        .iter()
        .map(|e| quote_ident(&e.name))
        .collect::<Vec<_>>()
        .join(", ");
    let vals = values
        .iter()
        .map(|e| render(&e.value))
        .collect::<Vec<_>>()
        .join(", ");
    Ok(format!(
        "INSERT INTO {}.{} ({}) VALUES ({});",
...
mod tests {
    ...
    fn pk_int(name: &str, n: i64) -> PrimaryKey {
        PrimaryKey {
            columns: vec![(name.into(), LiteralValue::Number(n.to_string()))],
        }
    }
    ...
    fn update_simple() {
        let edits = vec![ColumnEdit {
            name: "email".into(),
            value: LiteralValue::Text("ada@x.io".into()),
        }];
        let sql = build_update("public", "users", &edits, &pk_int("id", 1042)).unwrap();
        assert_eq!(
            sql,
            "UPDATE \"public\".\"users\" SET \"email\" = E'ada@x.io' WHERE \"id\" = 1042;"
        );
    }
    ...
    fn update_with_quote_in_text_is_escaped() {
        let edits = vec![ColumnEdit {
            name: "name".into(),
            value: LiteralValue::Text("O'Brien".into()),
        }];
    ...
    fn insert_simple() {
        let values = vec![
            ...
            ColumnEdit {
                name: "email".into(),
                value: LiteralValue::Text("a@b.c".into()),
            },
        ];
    ...
    fn bytes_render_as_hex() {
        let edits = vec![ColumnEdit {
            name: "data".into(),
            value: LiteralValue::Bytes(vec![0xde, 0xad, 0xbe, 0xef]),
        }];
        let sql = build_update("p", "t", &edits, &pk_int("id", 1)).unwrap();
