...
impl TableInspectorView {
    ...
    pub fn rows_view(&self) -> &RowsView {
        &self.rows
    }
    ...
    fn apply(&mut self, payload: ViewPayload) {
        match payload {
            ...
            ViewPayload::Columns(Ok(cols)) => {
                let display: Vec<Vec<String>> = cols
                    .iter()
                    .map(|c| {
                        vec![
                            c.name.clone(),
            ...
            ViewPayload::Constraints(Ok(con)) => {
                let display: Vec<Vec<String>> = con
                    .iter()
